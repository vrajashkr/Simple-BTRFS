...
int main()
{
        ...
        while (rogue)
        {
                ...
                case 0:
                {
                        fx_ls(stack_top(&dirstack)->dir_tree);
                }
                break;
                case 1:
                {
                        fx_cd(&dirstack, words[1]);
                }
                break;
                case 2:
                {
                        fx_mkdir(&dirstack, stack_top(&dirstack), words[1]);
                }
                break;
                case 3:
                {
                        fx_rmdir(&dirstack, stack_top(&dirstack), words[1]);
                }
                break;
                case 4:
                {
                        fx_cat(stack_top(&dirstack)->dir_tree, words[1]);
                }
                break;
                case 5:
                {
                ...
                case 6:
                {
                        fx_touch(&dirstack, stack_top(&dirstack), words[1], "abc");
                }
                break;
                case 7:
                {
                        fx_rm(&dirstack, stack_top(&dirstack), words[1]);
                }
                break;
                case 8:
                {
                ...
                default:
                {
                        printf("[ERROR] Is it that hard to follow instructions? That command doesn't exist\n");
                }
                break;
                }
        }
...
void insert_leaf(STACKQUEUE *s, BNODE *cr, char *target, long id)
{
        ...
        { //leaf
                if (current_root->length < 2)
                {
                        //there is some space
                        int i = 0;
                        printf("len %d", current_root->length);
                        fflush(stdout);
                        for (i = 0; i < current_root->length + 1; i++)
                        {
                                //printf("i %d\n",i);fflush(stdout);
                                printf("compare: %s %s %d \n", target, current_root->names[i], strcmp(target, current_root->names[i]));
                                fflush(stdout);
                                if (strcmp(target, current_root->names[i]) < 0)
                                {
                                        //create space and insert
...
void fx_rm(STACKQUEUE *s, DIR *current_root, char *name)
{
        ...
        if (cond)
        {
                printf("[NOTE] File %s successfully removed", name);
        }
        else
        {
